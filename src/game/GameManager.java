...
public class GameManager {
	...
	//Check if livePlayers is null;
	private boolean isLivePlayersNull;
	...
	public void endTurn() {
		...
		if(allHome) {
			...
			//Check if mainPlayer won
			//If true, play victory sound
	...
	public void nextPlayer() {
		...
		if(currentPlaying == mainPlayer)
		{
			System.out.println("YOUR TURN: " + livePlayers[mainPlayer].getColor());
			new DJ("your_turn");
		}
	...
	public boolean tileClicked(Tile tile, Player player) {
		...
		if(card.getType() == CardDeck.SEVEN && sevenRemainder != -1) {
			...
			animateTravel(tile, pawn, sevenRemainder);
			...
			animateTravel(tile, pawn, CardDeck.getValue(card));
...
//			if(!start.isOccupied())
//			{
//				System.out.println("Moving animated pawn.......");
//				mGamePanel.moveAnimatedPawn(pawn, start.getX(), start.getY());	
//			}
			
			if(start.isOccupiedByColor(pawn.getColor())) {
				return null;
			}
	...
	public void animateTravel(Tile start, Pawn pawn, int numSpaces) {
		//OutMail.send(MessageConstants.BROADCAST, MessageConstants.STOP_TIMER);
	...
	public void deactivatePlayer(String name) {
		...
		if(isLivePlayersNull == false && players[index] == livePlayers[currentPlaying]) {
