...
public class ColorSelector extends PaintedPanel{
	...
	public ColorSelector(ActionListener confirmAction, Image inImage) {
		...
		countdownLabel = new JLabel("Time 0:" + time, JLabel.CENTER);
		countdownLabel.setFont(FontLibrary.getFont("fonts/kenvector_future_thin.ttf", Font.PLAIN, 28));
		countdownLabel.setOpaque(true);
		
		ClearPanel countdownPanel = new ClearPanel();
		countdownPanel.setBorder(new EmptyBorder(new Insets(30, 0, 0, 0)));
	...
	class CountdownListener implements ActionListener 
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			if(--time > 0)
			{
				String textDisplay = "Time 0:";
				if(time < 10)
				{
					textDisplay += "0";
				}
				textDisplay += time;
				countdownLabel.setText(textDisplay);
			}
			else
			{
